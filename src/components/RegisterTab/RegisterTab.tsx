...
export default class RegisterTab extends React.Component<RegisterTabProps, RegisterTabState> {
    render() {
        return (
            <>
                <Backdrop sx={{ color: '#fff', zIndex: (theme) => theme.zIndex.drawer + 1 }} open={this.props.visible}>
                    ...
                    <Close className={`absolute top-5 right-[285px] cursor-pointer text-black`} style={this.props.visible? {display: 'block'} : {display: 'none'}} onClick={this.props.onClose} />
                </Backdrop>
                {this.props.children}
            </>
        )
